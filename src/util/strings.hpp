...
namespace big
{
	...
	inline auto to_lower_case = [](std::string text) {
	...
	std::vector<T> filterStrings(
	    const std::vector<T>& inputVector, const std::string& searchString, std::string (*reducer)(T&) = [](std::string& t) {
		    return t;
	    })
	{
		...
		for (auto some : inputVector)
		{
			...
			std::string lowercaseStr = to_lower_case(reducer(some));
	...
	inline std::map<std::string, std::string> filterStrings(const std::map<std::string, std::string>& inputMap, const std::string& searchString, bool filterByKey)
	{
		...
		{
			// Convert the current string to lowercase for case-insensitive comparison
			std::string lowercaseStr = to_lower_case(filterByKey ? pair.first : pair.second);
