		...
		virtual void execute(player_ptr player, const command_arguments& _args, const std::shared_ptr<command_context> ctx) override
		{
			...
			if (auto net_data = player->get_net_data())
			{
				...
				if (!player->infractions.empty()
				    && (player->infractions.contains((int)Infraction::TRIED_KICK_PLAYER) || player->infractions.contains((int)Infraction::TRIED_CRASH_PLAYER)) && !bad_players_nm::is_blocked(rockstar_id))
